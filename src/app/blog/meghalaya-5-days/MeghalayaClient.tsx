...
function ShareBar() {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {[
        { label: "Email", color: "bg-ink text-white", href: `mailto:?subject=Meghalaya 5-Day Itinerary&body=Check this out: ${pageUrl}` },
        { label: "Twitter", color: "bg-[#1a6fb5] text-white", href: `https://x.com/intent/tweet?text=Meghalaya in 5 Days guide&url=${pageUrl}` },
      ].map((s) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              {[
                ...
                <div key={d.days} className={`rounded-xl border p-5 ${d.color}`}>
                  ...
                  <p className="text-xs text-gray-700 font-light leading-relaxed">{d.desc}</p>
                </div>
              ))}
            </div>
          </section>
